...
echo "$($PSQL "select (sum(winner_goals)+sum(opponent_goals))/count(*)::decimal from games")"
...
echo "$($PSQL "select distinct(name) from teams left join games on teams.team_id = games.winner_id or teams.team_id = games.opponent_id where round='Eighth-Final' and year = 2014")"
